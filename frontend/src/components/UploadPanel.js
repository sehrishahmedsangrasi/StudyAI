...
export default function UploadPanel({ onGenerated, onLoading }) {
  ...
  const handleGenerate = async () => {
    // 1. Validation: Ensure we have a file and a token
    ...
    try {
      // Step 1: Upload & ingest to /files/upload
      ...
      const up = await axios.post(`${API}/files/upload`, form, {
        headers: { 
          ...
          'Authorization': `Bearer ${token}` // Sends your Google User ID to the backend
        },
        timeout: 60000,
      });

      // Get the unique file_id generated by your backend
      const fileId = up.data.file_id;
      setUploadMsg(`File uploaded successfully!`);
      setStep('Generating notes, quiz & flashcards...');

      // Step 2: Generate all content using the /generate endpoint
      // This matches your FastAPI @app.post("/generate") payload
      const gen = await axios.post(`${API}/generate`, {
        file_ids: [fileId], // Your backend expects a list of IDs
      ...
      if (err.code === 'ERR_NETWORK') {
        setError('Cannot reach server. It may be waking up (takes ~1 min on Render free tier).');
  ...
  return (
    <div style={styles.panel}>
      ...
      <div style={{ marginTop: 16, padding: '10px 14px', background: '#0a0a0f', borderRadius: '10px', border: '1px solid #2a2a3a', fontSize: '0.78rem', color: '#9090a8' }}>
        Backend: <a href={API} target="_blank" rel="noreferrer" style={{ color: '#6c63ff' }}>http://127.0.0.1:8000</a> — must show JSON to work
      </div>
